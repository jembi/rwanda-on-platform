...
if [[ ! " ${valid_actions[*]} " =~ " ${action} " ]]; then
  ...
  echo "Usage: deploy-local.sh {action} [linux|macos|windows]"
  exit 0
fi
...
linux)
  ...
  ./instant-linux package "$action" --profile=local --concurrency=10 "${@:3}"
  exit 0
  ;;
macos)
  echo "Running with the MacOS CLI binary"
  ./instant-macos package "$action" --profile=local --concurrency=10 "${@:3}"
  exit 0
  ;;
windows)
  echo "Running with the Windows CLI binary"
  ./instant.exe package "$action" --profile=local  --concurrency=10 "${@:3}"
  exit 0
  ;;
--help)
  echo "Usage: deploy-local.sh {action} [linux|macos|windows]"
